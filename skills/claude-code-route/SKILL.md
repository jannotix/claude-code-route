...
description: Enterprise engineering discipline as a three-role cycle — plan, execute, review, repeat — that delivers code which was actually run, at the token cost of an ordinary session, and records who did what with which model in an append-only project history. Use for any substantive software work: new capabilities, refactors, defects, API, schema or contract changes, migrations, architecture questions, and work on codebases too large to read. Also whenever the user mentions spec-driven development, SDD, domain-driven design, DDD, bounded context, aggregate, ubiquitous language, invariant, acceptance criteria, traceability, ADR, audit trail, project history, cross-model review, or asks for senior, production-grade or enterprise-grade engineering. Governs output voice — English comments only where they carry what the code cannot, no narration in code, artifacts or reports.
